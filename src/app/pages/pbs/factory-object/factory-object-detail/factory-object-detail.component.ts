      ...
      next: (res) => {
        ...
        if (0 === res.Code) {
          ...
          if (this.factoryObjectUpdate.FactoryObjectProperties !== null) {
            // 找出类静态属性
            this.typePropertyList = this.factoryObjectUpdate.FactoryObjectProperties?.filter(property => property.PropertyCategory === '类静态')!;
            // 找出对象静态属性
            this.objectPropertyList = this.factoryObjectUpdate.FactoryObjectProperties?.filter(property => property.PropertyCategory === '对象静态')!;
            // 找出运行属性
            this.runPropertyList = this.factoryObjectUpdate.FactoryObjectProperties?.filter(property => property.PropertyCategory === '运行')!;
          }
        }
      },
      error: (error) => {
